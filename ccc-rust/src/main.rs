...
fn long_options()-> Options{
    ...
    opts.optopt("p","port", "Specifies the gRPC server port to connect to.","NUMBER");
	opts.optopt("c","command","Specifies the command to use","COMMAND");
    opts.optflag("h", "help", "Displays a general help or a help message on the command.");
	opts.optflag("l", "list", "Displays the available methods.");
	opts.optflag("v", "version", "Displays the version of ccc.");
	opts.optflag("n", "nocolor", "Outputs are displayed with the current color.");
	opts
}
