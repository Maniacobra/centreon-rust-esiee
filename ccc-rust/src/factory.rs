...
{
    let v_slowest_statements_count = j_data.get("slowest_statements_count");
    let v_slowest_queries_count = j_data.get("slowest_queries_count");

    let msg = SqlManagerStatsOptions {
        slowest_statements_count: match v_slowest_statements_count {
            Some(v) => Some(v.as_u64().unwrap() as u32),
            None => return None,
        },
        slowest_queries_count: match v_slowest_queries_count {
            Some(v) => Some(v.as_u64().unwrap() as u32),
            None => return None,
        },
    };
